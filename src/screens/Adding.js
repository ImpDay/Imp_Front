    ...
    <SafeAreaView
          ...
          <View
            ...
            <View
              ...
              <TouchableOpacity onPress={handleAddItem}>
                <AntDesign
                  name="plus"
                  style={{color: '#1133ff', fontWeight: 'bold', fontSize: 17}}
                />
              </TouchableOpacity>
            </View>
          </View>
      ...
      <View style={{backgroundColor: 'black', width: '100%', height: '10%'}}>
        <View
          ...
          <TouchableOpacity
            onPress={() => {
              createNewTemplate(myArray, period, title);
            ...
            <View
              ...
              <Text
                style={{color: '#dddddd', fontWeight: 'bold', fontSize: 15}}>
                Save
              </Text>
            </View>
          </TouchableOpacity>
...
async function createNewTemplate(questionList, writePeriod, templateName) {
  // TODO: Record 만들기
  ...
  const url = `http://172.10.5.148:443/templates`;
  let newTemplateId;
  try {
    // create record
    const response = await axios.post(url, postData);
    console.log('This is new template Id : ' + response.data.templateId);
    newTemplateId = response.data.templateId;
  } catch (error) {
    console.error(error);
  }
  for (const questionData of questionList) {
    const postAnswerUrl = `http://172.10.5.148:443/questions`;
    const answerData = {
